function  [ NL, EL ] = mesh_3 (d1,d2,p,m ,R)
%TR element mesh
...
NL = zeros(NoN,PD);
 ...
 for i = 1: p+1 %% nodes at the top of region 2
...
for i= 1:m
    for j= 1:2:8*p
        if j==1  %the first element in strip
            ...
            EL((i-1)*(8*p)+j,1) = (i-1)*(4*p)+1;   %first node is same as square
            %EL((i-1)*(4*p)+j,2) = EL((i-1)*(4*p)+j,1) + 1    ; %2nd node
            EL((i-1)*(8*p)+j,3) = EL((i-1)*(8*p)+j,1) + 4*(p)   ; %4th node of square is 3rd of triangle
            EL((i-1)*(8*p)+j,2) = EL((i-1)*(8*p)+j,1) + 4*(p) + 1    ; %3rd node of square is 2nd node of triangle
            
            EL((i-1)*(8*p)+j+1,1) = (i-1)*(4*p)+1;   %first node is same as 1st node of previous
            %EL((i-1)*(4*p)+j+1,2) = EL((i-1)*(4*p)+j,1) + 1    ; %2nd node
            EL((i-1)*(8*p)+j+1,3) = EL((i-1)*(8*p)+j,2)   ; %3rd node is 3rd node of previous
            EL((i-1)*(8*p)+j+1,2) = EL((i-1)*(8*p)+j+1,1) + 1    ; %2nd node is 1st node of this + 1
            
        elseif j==8*p - 1 %if last element in strip
            %note that we use i not i-1 since we want the contribution of 4*p from the get go
            EL((i-1)*(8*p)+j,1) = (i-1)*(4*p) + 1;   %first element is last row's first element we could say
            %EL((i-1)*(4*p)+j,4) = EL((i-1)*(4*p)+j,1) + 4*p - 1 ; %4th element is 1'st element + 1 row - 1 element
            EL((i-1)*(8*p)+j,3) = EL((i-1)*(8*p)+j,1) + 4*p - 1 ; %2nd node is 1st node + 4*p
            EL((i-1)*(8*p)+j,2) = EL((i-1)*(8*p)+j,1)+ 4*p + 4*p - 1 ;%  3rd element is 4th element + 1 row (4*p)
            
            EL((i-1)*(8*p)+j+1,1) = (i-1)*(4*p) + 1;   %first element is last row's first element we could say
            %EL((i-1)*(4*p)+j+1,4) = EL((i-1)*(4*p)+j+1,1) + 4*p - 1 ; %4th element is 1'st element + 1 row - 1 element
            EL((i-1)*(8*p)+j+1,2) = EL((i-1)*(8*p)+j+1,1) + 4*p ; %2nd node is 1st node + 4*p
            EL((i-1)*(8*p)+j+1,3) = EL((i-1)*(8*p)+j+1,2)+ 4*p - 1 ;%  3rd element is 4th element + 1 row (4*p)
            
        else
            
            EL((i-1)*(8*p)+j,1) = EL((i-1)*(8*p)+j-1,2)  ;  %first node of current element is 2nd node from prev element
            EL((i-1)*(8*p)+j,3) = EL((i-1)*(8*p)+j-1,3)  ;  % 3rh node of current element is 3rd element from prev element
            EL((i-1)*(8*p)+j,2) = EL((i-1)*(8*p)+j,3) + 1;  %2nd node of current element is 3rd node of current element + 1
            %EL((i-1)*(4*p)+j,2) = EL((i-1)*(4*p)+j,1) + 1;  %2nd node of curren element is 1st node of current element + 1
            
            EL((i-1)*(8*p)+j+1,1) = EL((i-1)*(8*p)+j,1)  ;  %first node of current element is same as 1st of previous
            %EL((i-1)*(4*p)+j+1,4) = EL((i-1)*(4*p)+j-1,3)  ;  % 4th node of current element is 3rd element from prev element
            EL((i-1)*(8*p)+j+1,3) = EL((i-1)*(8*p)+j,2);  %3rd node of current element is 4th node of current element + 1
            EL((i-1)*(8*p)+j+1,2) = EL((i-1)*(8*p)+j+1,1) + 1;  %2nd node of curren element is 1st node of current element + 1

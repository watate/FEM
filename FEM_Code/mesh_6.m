function [NL,EL] = mesh_6(d1,d2,p,m,R) 
tic
COC = [d1/2 d2/2] ;
m = 2*m ;
%assembling the NL 
...
% make points in the center, add to the node list end
for i = 1 : size(EL,1)
     x = NL(EL(i,1),1) + (NL(EL(i,3),1) - NL(EL(i,1),1))*(0.5) ;
     y = NL(EL(i,1),2) + (NL(EL(i,3),2) - NL(EL(i,1),2))*(0.5) ;
     NL = [NL ; x y] ;
end
o = 1;
%odd numbered: 
ELL = [o (2*4*p+1+o) (2*4*p+o) (2*4*p*m + 4*p + (4*p*m + 4*p) + o) (2*4*p*m + 2*4*p+o) o+4*p ] ;
j = 1;
for i = 2 : 2*2*p*m
 ELL(i,:) = ELL(i-1,:) + o ;
 %last
 for k = 1: size(ELL,1)
        ...
        if ELL(k,1) == 4*p*j
         ELL(k,2) = ELL(k-1,2) - (4*p-1);
        end
 
 %first
        if ELL(k,1) == 4*p*j + 1 
            ELL(k,1) = ELL(k-1,1) + (4*p+1);
            ELL(k,2) = ELL(k-1,2) + (2*4*p+1);
            ELL(k,3) = ELL(k-1,3) + (4*p+1);
            ELL(k,6) = ELL(k-1,6) + (4*p+1);
     j = j + 2; 
        end
...
%even numbered EL 
j =1 ;
ELR = [o (o+1) (2*4*p+1+o) (2*4*p*m + 4*p + o) (o+1+4*p) (2*4*p*m + 4*p + (4*p*m + 4*p) + o) ] ;
for i = 2 : 2*2*p*m
 ELR(i,:) = ELR(i-1,:) + o ;
 %last
 for k = 1: size(ELR,1)
        ...
        if ELR(k,1) == 4*p*j
         ELR(k, 2) = ELR(k-1,2) - (4*p-1) ; 
         ELR(k, 3) = ELR(k-1,3) - (4*p-1) ; 
         ELR(k, 5) = ELR(k-1,5) - (4*p-1) ; 
        end
 
 %first
        if ELR(k,1) == 4*p*j + 1 
            ELR(k,1) = ELR(k-1,1) + (4*p+1);
            ELR(k,2) = ELR(k-1,2) + (2*4*p+1);
            ELR(k,3) = ELR(k-1,3) + (2*4*p+1);
            ELR(k,5) = ELR(k-1,5) + (2*4*p+1);
     j = j + 2; 
        end
